...
public class ModuleParser extends AbstractXmlParser<Module> {
	...
	@Override
	public Module onCreateItem(final String rootPath) throws XPathExpressionException {
		String name;
		int credits;
		int sws;
		String responsible;
		...
		credits = ((Double) findByXPath(rootPath + "/credits/text()",
				XPathConstants.NUMBER, Number.class)).intValue();
		...
		for (int indexTeacher = 1; indexTeacher <= countTeachers; indexTeacher++) {
			...
			if (!StringUtil.isBlank(teacher)) {
				teachers.add(new String(teacher));
			}
		}
		...
		for (int indexCodes = 1; indexCodes <= countCodes; indexCodes++) {
			...
			for (int indexSemester = 1; indexSemester <= countSemester; indexSemester++) {
				...
				if (!StringUtil.isBlank(semester)) {
					semesterList
					.add(new String(Semester.of(Integer.parseInt(semester)).toString()));
				}
			}
